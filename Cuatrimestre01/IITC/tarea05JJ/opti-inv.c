...
void dgemm_avx(int n, double* A, double* B, double* C) {
    for (int j = 0; j < n; j++) {           // Invertimos el orden: ahora 'j' va primero
        for (int k = 0; k < n; k++) {       // Luego 'k'
            __m256d b0 = _mm256_broadcast_sd(&B[k + j * n]); // Broadcast de B[k][j]

            for (int i = 0; i < n; i += 4) { // Finalmente 'i' se itera en múltiplos de 4
...
int main() {
    int n = 4096; // Tamaño de la matriz debe ser múltiplo de 4 para AVX
    srand(time(NULL));
